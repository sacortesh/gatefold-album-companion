...
- TrackList — the tracklist column: track rows (number/title/artist/duration/TriageButton × 2), no longer a `TrackRow` with 6 booleans — split into an explicit `variant: "default" | "now-playing" | "selected"` plus separate `liked`/`inBanger` state props (2, not folded into a variant, since they're independently toggleable states, not mutually exclusive display modes). Phase 10.20 (scoped, not yet built): a "popular" `Badge` on a track's top-N (proposed 3) most-played rows per Last.fm, silent when Last.fm isn't configured.
- LyricsPanel — unchanged structurally (already well-scoped: 4 props, 2 boolean, under the proliferation threshold), restyled onto the new type scale. Phase 10.19 (scoped, not yet built): an explicit auto-follow toggle next to the "Lyrics: {track}" heading — on by default, clicking a track row turns it off, clicking the toggle turns it back on. Phase 11 (scoped in `docs/implementation-plan.md`, not yet built): a gloss-cycle control for non-Latin-script lyrics — romanization (11.1), furigana (11.2), and eventually translation (11.5) all share one toggle rather than stacking, shown only when the source script isn't already Latin-adjacent.
